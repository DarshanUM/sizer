    ...
    def process_csv_result(self, csv_res):
        ...
        for server_key, server_value in csv_res.items():
            for cluster, cluster_data in server_value.items():
                ...
                sum_vpcus += host_details['vcpus']

        wl_template['cpu_clock'] = round(sum_cpu_clock)
        wl_template['hdd_size'] = round(sum_hdd_size/1024)
        wl_template['ram_size'] = round(sum_ram_size/1024)
        wl_template['vcpus'] = round(sum_vpcus)
        return wl_template
...
class ClaimProfileData(BaseView):
    ...
    def post(self, request):
        ...
        error_msg = scenario_solve_obj.process_scenario_sizing(profiler_json_data, work_load)
        if error_msg:
